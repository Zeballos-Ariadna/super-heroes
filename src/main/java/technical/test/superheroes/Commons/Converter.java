...
public class Converter {
    ...
    /**
     * Conviernto un JSON String a una lista de instancia de una clase A
     *
     * @param from    El JSON string que se quiere instanciar
     * @param toClass La clase A que representa al contenido del JSON String
     * @param <T>     Clase destino parametrizable
     * @return Lista de instancia de clase A
     */
    public static <T> List<T> convertValues(String from, Class<T> toClass) {
        if (Objects.isNull(from)) {
            return null;
        }
        try {
            JavaType listType = mapper.getTypeFactory().constructCollectionType(List.class, toClass);
            mapper.configure(FAIL_ON_UNKNOWN_PROPERTIES, false); //FAIL_ON_UNKNOWN_PROPERTIES Objeto Origen tiene mas atributos que el objeto destino y esto permite haciendo solo para las propiedades que se parezcan
            mapper.configure(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY, true);
            return mapper.readValue(from, listType);
        } catch (Exception e) {
            LOGGER.severe("Error al intentar mappear un objeto de la clase " + from.getClass() + "a la clase " + toClass + ". \n Mensaje de error: " + e.getCause().getMessage());
            return null;
        }
    ...
    public static <T> List<T> convertValue(Iterable from, Class<T> toClass) {
        try {
            ...
            LOGGER.severe("Error al intentar mappear un objeto de la clase " + from.getClass() + "a la clase " + toClass + ". \n Mensaje de error: " + (Objects.nonNull(e.getCause()) ? e.getCause().getMessage() : ""));
            return null;
        }
    }
    ...
    public static <T> String encodeToBase64String(T fromObject) {
        String objectStrig = convertValue(fromObject);
        return new String(Base64.getEncoder().encode(objectStrig.getBytes()));
    }

    public static <T> T convertBase64ToClass(String uid, Class<T> type) {
        return Converter.convertValue(new String(Base64.getDecoder().decode(uid)), type);
    }
}
